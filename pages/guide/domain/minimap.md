...
:::note
**클라이언트 리소스팩가 필요하다.** 맵 픽셀의 특정 인덱스에 **셰이더가 읽는 제어값**으로 인코딩된다. 이를 해석하는 클라이언트 셰이더가 없으면 미니맵이 보이지 않는다.
:::
...
지나간 청크만 지도에 드러내고 나머지는 회색 안개로 가리는 기능. **개발 서버에서 청크 PDC 에 데이터가 쌓이는 것을 막기 위해 기본값은 `false`** 다.

```kotlin
service.setExplorationEnabled(true)
service.isExplorationEnabled()
```

:::warning
아직 실험적 기능입니다. 향후 지형 자체를 단색으로 표시하지 않고, 마커 렌더링을 활용하여 안개 셰이더를 표시할 방법을 고안 중입니다.
:::
