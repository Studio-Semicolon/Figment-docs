...
- **빌드타임 SSR (패킷 없음)** — 서버가 스택을 만들 때 viewer 로 빌드. GUI·보상·키트·상점처럼 **한 명만 보는** 스택에 쓴다. 위 `render(id, player)` 가 이것. 빌드 전체가 viewer 함수이므로 attribute·durability 등 **게임플레이 값까지 포함해 옵션 전체**가 그 플레이어 기준으로 갈린다. 이미 빌드 시점에 누구에게 줄지 알고 있으니 패킷까지 갈 필요 없음.
...
- **Stateless**(대부분): `migrators` 를 비워 둔다. 접속 시 통째로 재렌더된다. 수량과 **내구도(damage)는 자동 보존**되므로, 내구도만 지키면 되는 무기·도구도 그냥 stateless 로 둬도 된다.
...
- 갱신 시점: 현재 **PlayerJoin**. "온라인 중 인벤토리 열 때"는 향후 추가 예정.
